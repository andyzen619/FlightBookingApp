...
public class ItinerarySort {
    ...
    public static ArrayList<Flight> sortFlightCost(ArrayList<Flight> flights) {
      if (flights.size() < 2) {
        ...
        ArrayList<Flight> listOne = new ArrayList<Flight>(flights.subList(0, midpoint));
        ArrayList<Flight> listTwo = new ArrayList<Flight>(flights.subList(midpoint, flights.size()));
   ...
   * @return A sorted list
   */
  private static ArrayList<Flight> sortFlightList(ArrayList<Flight> sortOne, ArrayList<Flight> sortTwo) {
    ...
    if (counter == sortOne.size()) {
      ArrayList<Flight> sublist = new ArrayList<Flight>(sortTwo.subList(size, sortTwo.size()));
      sortedList.addAll(sublist);
    } else {
      ArrayList<Flight> sublist = new ArrayList<Flight>(sortOne.subList(counter, sortOne.size()));
      sortedList.addAll(sublist);
    }
    return sortedList;
  }
   ...
   * @return A sorted list.
   */
  public static ArrayList<FlightItinerary> sortItineraryCost(ArrayList<FlightItinerary> itineraries) {
    if (itineraries.size() < 2) {
      ...
      // separate them into two different list to sort them
      ArrayList<FlightItinerary> listOne = new ArrayList<FlightItinerary>
              (itineraries.subList(0, midpoint));
      ArrayList<FlightItinerary> listTwo = new ArrayList<FlightItinerary>
              (itineraries.subList(midpoint, itineraries.size()));
  ...
  private static ArrayList<FlightItinerary> sortItineraryList(ArrayList<FlightItinerary> sortOne,
    ...
    // just append the rest of the list depending what was emptied before
    if (counter == sortOne.size()) {
      ArrayList<FlightItinerary> sublist = new ArrayList<FlightItinerary>
              (sortTwo.subList(size, sortTwo.size()));
      sortedList.addAll(sublist);
    } else {
      ArrayList<FlightItinerary> sublist = new ArrayList<FlightItinerary>
              (sortOne.subList(counter, sortOne.size()));
      sortedList.addAll(sublist);
    }
    return sortedList;
  }
  ...
  public static ArrayList<Flight> sortFlightTravelTime(ArrayList<Flight> itineraries) {
    if (itineraries.size() < 2) {
      ...
      ArrayList<Flight> listOne = new ArrayList<Flight>(itineraries.subList(0, midpoint));
      ArrayList<Flight> listTwo = new ArrayList<Flight>
              (itineraries.subList(midpoint, itineraries.size()));
